...
import { ArrowLeft, FileText, Mail, DollarSign, AlertCircle, CheckCircle2, Clock, Trash2, CreditCard, PenTool } from "lucide-react";
...
interface InvoiceDetailScreenProps {
  ...
  onTakePayment?: () => void;
  onUpdateSignature?: (invoiceId: string, signature: string | null) => Promise<void>;
}
...
export function InvoiceDetailScreen({
  ...
  const [showSignaturePad, setShowSignaturePad] = useState(!!invoice.signature);
  ...
  const handleSignatureSave = async () => {
    ...
    try {
      await onUpdateSignature(invoice.id, signature);
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        ...
        <Card className="p-6">
          ...
          {businessData.contactEmail && (
            <div className="border-t pt-4 mt-4">
              <h3 className="text-sm uppercase text-muted-foreground mb-2">Contact At</h3>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            {onUpdateSignature && (
              <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      if (confirm("Are you sure you want to remove the signature?")) {
                        await onUpdateSignature(invoice.id, null);
                        setShowSignaturePad(false);
                      }
                    }}
                  >
                ...
                {showSignaturePad && !invoice.signature && (
                  <Button
                    variant="outline"
                    size="sm"
          ...
          {invoice.signature ? (
            ...
            <div 
              className="border border-dashed border-border rounded-lg p-6 text-center text-muted-foreground cursor-pointer hover:bg-muted/30 transition-colors"
              onClick={() => onUpdateSignature && setShowSignaturePad(true)}
            >
              No signature added yet - Click to add signature
            </div>
          )}
        </Card>
        ...
        <div className="space-y-3">
          ...
          {isPending && onTakePayment && (
            <Button
              className="w-full bg-gradient-to-r from-[#1E3A8A] to-[#14B8A6] hover:opacity-90 text-white"
