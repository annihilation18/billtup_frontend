...
export function ForgotPasswordScreen({ onBack }: ForgotPasswordScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Send password reset request to our backend (which uses Nodemailer)
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-dce439b6/auth/request-password-reset`, {
        method: 'POST',
        headers: {
