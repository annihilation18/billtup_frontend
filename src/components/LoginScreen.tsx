...
import { Building2, Mail, Lock, AlertCircle, Phone, Briefcase, Percent } from "lucide-react";
...
interface LoginScreenProps {
  onLoginSuccess: (isNewSignup: boolean) => void;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Pass true for isNewSignup
        onLoginSuccess(true);
      } else {
        await authApi.signIn(email, password);
        toast.success("Welcome back!");
        // Pass false for isNewSignup (or omit as it defaults to false)
      ...
      } else if (errorMsg.includes('already been registered') || errorMsg.includes('User already registered')) {
