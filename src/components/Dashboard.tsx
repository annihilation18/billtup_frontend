...
import { Search, Plus, FileText, Users, Settings, Clock, CheckCircle2, DollarSign, Calendar, TrendingUp, BarChart } from "lucide-react";
...
export function Dashboard({ invoices, onCreateInvoice, onViewInvoice, currentTab, onTabChange, businessLogo, businessName }: DashboardProps) {
  ...
  useEffect(() => {
    const fetchMonthlyCount = async () => {
      try {
        ...
        // This is expected until you run: ./deploy-backend.sh
        // The app will use fallback calendar month dates instead
      }
    };
  ...
  // Get fallback billing period (calendar month) when backend data isn't available
  const getFallbackBillingPeriod = () => {
    const now = new Date();
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
          ...
          <Card className="p-3 bg-gradient-to-br from-[#1E3A8A]/10 to-[#1E3A8A]/5 border-[#1E3A8A]/20">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[10px] text-muted-foreground">{currentMonthName} Sales</p>
            </div>
          ...
          <Card className="p-3 bg-gradient-to-br from-[#8B5CF6]/10 to-[#8B5CF6]/5 border-[#8B5CF6]/20">
            ...
            <p className="text-lg font-mono text-[#8B5CF6]">
              {monthlyInvoiceCount ? monthlyInvoiceCount.invoiceCount : '—'}
            </p>
            {monthlyInvoiceCount && (
              <p className="text-[9px] text-muted-foreground mt-0.5">
                {monthlyInvoiceCount.billingPeriod.daysRemaining} days left
              </p>
            )}
          </Card>
        </div>
        
        {/* Billing Period Info - Removed duplicate, consolidated below search bar */}
      </div>
        ...
        <Card className="p-2 bg-muted/50 border-muted">
          <div className="flex items-center justify-between text-[10px]">
            ...
            <span className="font-mono">
              {monthlyInvoiceCount 
                ? `${monthlyInvoiceCount.billingPeriod.start} – ${monthlyInvoiceCount.billingPeriod.end}`
                : getFallbackBillingPeriod()}
            </span>
          </div>
        </Card>
